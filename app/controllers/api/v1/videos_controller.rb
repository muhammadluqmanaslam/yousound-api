   ...
   swagger_controller :videos, 'Videos'
      ...
      param :form, 'stream[spotlight_video]', :boolean, :optional
		...
		def create
			...
			upload_url = ''
			spotlight_video = params[:stream][:spotlight_video] rescue false
			Stream.where(user_id: current_user.id, spotlight_video: true).destroy_all if spotlight_video
			...
			begin
				...
				@stream.assign_attributes(
					...
					duration: duration,
					status: Stream.statuses[:active],
					spotlight_video: spotlight_video
				)
